...
| One-command startup | `scripts/win/start-no-docker.ps1` prepares the configured Windows runtime folders, runs pre-start verification, calls `run-dev.ps1`, then runs post-start verification |
...
| Runtime root | `CYBERFUSION_ENV_ROOT`, defaulting to an `Environment\cyberfusion-platform` folder next to the project parent, stores uploads, backend logs, backups, local VM evidence, Maven cache, pnpm store, and npm cache outside source |
...
Windows no-Docker mode expects the source project under an operator-chosen stable Windows folder and runtime data under `CYBERFUSION_ENV_ROOT` outside the source tree. It does not launch MySQL or Redis. Operators must start those two services first. The bundled SQL seed files currently create and seed the fixed database name `cyberfusion_soc`; custom database names require a manually prepared schema and `start-no-docker.ps1 -SkipDbInit`.
