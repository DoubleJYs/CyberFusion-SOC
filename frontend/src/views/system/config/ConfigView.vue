<template>
  <div class="page-shell">
    <SystemGovernanceHero
      title="参数配置"
      description="集中维护平台显示、安全阈值、导入策略等非敏感配置模板，避免真实密钥和运行数据进入源码。"
      scope="运行配置"
      :total="total"
    ...
    <el-card shadow="never">
      <DataTable
        ...
        :columns="columns"
        title="参数清单"
        subtitle="参数键、参数值、分组、可编辑状态和启停状态"
        badge="Config"
        :loading="loading"
        empty-text="暂无参数配置"
...
const configSummary = computed(() => [
  { label: '当前页参数', value: rows.value.length, hint: '已加载结果' },
  { label: '可编辑', value: rows.value.filter((row) => row.editable === 1).length, hint: '允许运维调整' },
  { label: '启用参数', value: rows.value.filter((row) => row.status === 1).length, hint: '当前生效' },
])
