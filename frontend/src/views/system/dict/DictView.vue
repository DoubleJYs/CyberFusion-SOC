<template>
  <div class="page-shell">
    ...
    <div class="dict-layout">
      ...
      <template #header><div class="card-header governance-table-header"><div><strong>字典类型</strong><span>枚举分类、编码和启停状态</span></div><el-button v-permission="'system:dict:create'" type="primary" :icon="Plus" @click="openType()">新增</el-button></div></template>
      ...
      <template #header><div class="card-header governance-table-header"><div><strong>字典数据</strong><span>当前字典类型下的标签、值、排序和状态</span></div><el-button v-permission="'system:dict:create'" type="primary" :icon="Plus" :disabled="!currentType" @click="openData()">新增</el-button></div></template>
...
const dictSummary = computed(() => [
  { label: '字典类型', value: types.value.length, hint: '枚举分类' },
  { label: '当前数据', value: dataRows.value.length, hint: currentType.value?.dictName || '未选择类型' },
  { label: '启用数据', value: dataRows.value.filter((row) => row.status === 1).length, hint: '可用于表单' },
])
