...
const integrationCards: IntegrationCard[] = [
  ...
  },
  {
    ...
    principles: [
      ...
      '结合告警降噪策略过滤重复、白名单和已知误报命中。',
    ],
    results: ['检测规则台账', '命中预览', '告警生成依据', '白名单和降噪策略'],
    relations: [
      { linkedTo: '告警中心', title: '规则命中生成告警依据', description: '事件字段匹配 Sigma 或本地规则后形成告警原因和命中解释。' },
      { linkedTo: '告警降噪', title: '白名单过滤低价值命中', description: '白名单、重复模式和已知误报会减少进入待处置队列的噪声。' },
      { linkedTo: '治理策略', title: '规则变更可审计', description: '规则、字段映射和降噪策略变更会保留命中预览与审计上下文。' },
    ],
    systemUse: [
      { title: '规则维护', description: '在检测规则中心查看规则来源、字段映射和命中结果。' },
      { title: '降噪治理', description: '在告警降噪中维护白名单，减少重复或低价值告警。' },
...
const algorithmCards: AlgorithmCard[] = [
  ...
  {
    key: 'algorithm-alert-noise',
    code: 'ALG-03',
    name: '告警降噪与白名单',
    category: 'Noise Reduction',
    description: '用白名单、重复模式和规则命中预览识别低价值告警，保留可解释的过滤原因。',
    inputs: ['规则命中', '字段白名单', '重复告警', '历史误报'],
    outputs: ['降噪命中', '保留理由', '误报过滤'],
    usedBy: '检测规则中心 / 告警降噪 / 告警中心',
    target: { path: '/soc/alert-noise' },
    targetLabel: '进入告警降噪',
    tone: 'blue',
    principles: [
      '读取规则命中字段、白名单条件、历史误报和重复告警特征。',
      '先判断是否命中明确白名单，再对重复模式和低价值模式做降噪标记。',
      '保留过滤原因和命中字段，便于审计为什么某条告警未进入优先队列。',
    ],
    results: ['降噪命中记录', '白名单匹配原因', '误报过滤结果', '保留告警列表'],
    relations: [
      { linkedTo: '检测规则中心', title: '规则命中提供依据', description: '规则字段和命中结果决定降噪判断的输入粒度。' },
      { linkedTo: '告警中心', title: '减少待处置噪声', description: '降噪后只把需要分析员关注的告警推入主列表。' },
      { linkedTo: '治理策略', title: '白名单变更可回溯', description: '白名单和过滤策略变更会影响命中预览和审计记录。' },
    ],
    systemUse: [
      { title: '治理误报', description: '分析员在告警降噪页查看哪些告警被过滤及原因。' },
      { title: '调整白名单', description: '确认业务正常流量后，维护字段白名单减少重复告警。' },
      { title: '回看影响', description: '通过命中预览验证新策略是否误伤高价值告警。' },
    ],
  },
  {
