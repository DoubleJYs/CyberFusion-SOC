...
Host Agent 的源码在 `agent/`，安装脚本在 `scripts/mac/` 和 `scripts/win/`。Agent token 由平台 `/api/soc/agents/register` 生成，服务端只保存 hash，明文 token 只在安装时显示一次，应放入本机运行目录的 `agent.env` 或操作系统密钥保护机制。
...
| `CYBERFUSION_AGENT_FIM_PATH` | 否 | `/etc/hosts` 或受控测试文件 | 可选 FIM hash 元数据采集目标，不上传文件正文 |
